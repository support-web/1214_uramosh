...
export default function RegisterPage() {
  ...
  const defaultRole = searchParams.get('role') === 'diviner' ? 'DIVINER' : 'CLIENT';
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      if (signInResult?.ok) {
        if (data.role === 'DIVINER') {
          router.push('/diviner/setup');
        } else {
          router.push('/');
        }
        router.refresh();
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <Link href="/" className="inline-flex items-center justify-center gap-2 mb-4">
            ...
            <span className="text-2xl font-bold text-gray-900">ウラナビ</span>
          </Link>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <span className="text-sm font-medium">相談者として登録</span>
                <span className="text-xs text-gray-500 mt-1">占いを利用する</span>
              </button>
              <button
                type="button"
                onClick={() => setValue('role', 'DIVINER')}
                className={`relative flex flex-col items-center p-4 rounded-lg border-2 transition-all ${
                  selectedRole === 'DIVINER'
                    ? 'border-primary-600 bg-primary-50'
                    : 'border-gray-200 hover:border-gray-300'
                }`}
              >
                ...
                <span className="text-sm font-medium">占い師として登録</span>
                <span className="text-xs text-gray-500 mt-1">占いを提供する</span>
              </button>
            </div>
          </div>
